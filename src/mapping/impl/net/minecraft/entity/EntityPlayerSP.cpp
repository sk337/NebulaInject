...
EntityPlayerSP::EntityPlayerSP(Phantom *phantom, Minecraft *mc)
    : AbstractClass(phantom, "PlayerSP") {
  this->mc = mc;

  // Get all the field and method IDs for EntityPlayerSP that we want (mappings
  // are for 1.12)
  fdPosX = getFieldID("x");
  fdPosY = getFieldID("y");
  fdPosZ = getFieldID("z");
...
fdInventory = getFieldID("inventory");           // InventoryPlayer field
fdCurrentItem = getFieldID("currentItem");      // int field in InventoryPlayer
mdGetStackInSlot = getMethodID("getStackInSlot"); // ItemStack getStackInSlot(int)
  ...
  mdIChatComponentGetFmtTxt = phantom->getEnv()->GetMethodID(
      IChatComponent, "func_150254_d", "()Ljava/lang/String;");
}

Entity EntityPlayerSP::asEntity() {
    return Entity(phantom, getEntityPlayerSP()); // wrap the underlying jobject
}
jdouble EntityPlayerSP::getPosX() { return getDouble(mc->getPlayer(), fdPosX); }

jdouble EntityPlayerSP::getPosY() { return getDouble(mc->getPlayer(), fdPosY); }

jdouble EntityPlayerSP::getPosZ() { return getDouble(mc->getPlayer(), fdPosZ); }
...
jint EntityPlayerSP::getId() { return getInt(mc->getPlayer(), mdGetId); }
...
jdouble EntityPlayerSP::getMotionX() {
  return getDouble(mc->getPlayer(), fdMotionX);
}

jdouble EntityPlayerSP::getMotionZ() {
...
// Getters for motion components
jdouble EntityPlayerSP::getMotionY() {
  return getDouble(mc->getPlayer(), fdMotionY);
}

// Optionally, a combined setter for all motions
void EntityPlayerSP::setMotion(jdouble motionX, jdouble motionY,
                               jdouble motionZ) {
  setDouble(mc->getPlayer(), fdMotionX, motionX);
  setDouble(mc->getPlayer(), fdMotionY, motionY);
  setDouble(mc->getPlayer(), fdMotionZ, motionZ);
}

// New definition for two-axis motion
void EntityPlayerSP::setMotionHorizontal(jdouble motionX, jdouble motionZ) {
    setDouble(mc->getPlayer(), fdMotionX, motionX);
    setDouble(mc->getPlayer(), fdMotionZ, motionZ);
}
...
void EntityPlayerSP::setMotionY(jdouble motionY) {
  setDouble(mc->getPlayer(), fdMotionY,
            getDouble(mc->getPlayer(), fdMotionY) * motionY);
}
...
jobject EntityPlayerSP::getEntityPlayerSP() { return mc->getPlayer(); }
